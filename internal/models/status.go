...
var statusLabels = map[Domain]map[string]label{
	DomainOrder: {
		...
		OrderReadyToPlan:     {"Order siap diproses", "Order ready to be processed"},
		OrderAssigned:        {"Driver dipilih", "Driver selected"},
		OrderInTransit:       {"Dalam Perjalanan", "In Transit"},
		OrderDelivered:       {"Terkirim", "Delivered"},
		OrderCompleted:       {"Order Selesai", "Order Completed"},
	...
	// The driver-flow names, as the old order service (karlo_order,
	// common/constants/status.js) spelled them; the console and K-Trip use
	// the same words so a status reads identically everywhere.
	DomainShipment: {
		ShipmentAssigned:          {"Menunggu Driver Menerima Order", "Waiting for the driver to accept"},
		ShipmentToLoading:         {"Menuju Titik Muat", "Towards Loading Point"},
		ShipmentAtLoading:         {"Sampai di Titik Muat", "Arrived at Loading Point"},
		ShipmentLoadingApproved:   {"Muat disetujui PIC", "Load PIC approved"},
		ShipmentLoading:           {"Mulai Muat", "Start Loading"},
		ShipmentLoaded:            {"POD Muat Terverifikasi", "Load POD Verified"},
		ShipmentToUnloading:       {"Menuju Titik Bongkar", "Towards Unloading Point"},
		ShipmentAtUnloading:       {"Sampai di Titik Bongkar", "Arrived at Unloading Point"},
		ShipmentUnloadingApproved: {"Bongkar disetujui PIC", "Unload PIC approved"},
		ShipmentUnloading:         {"Mulai Bongkar", "Start Unloading"},
		ShipmentUnloaded:          {"POD Bongkar Terverifikasi", "Unload POD Verified"},
		ShipmentFinished:          {"Selesai Shipment", "Completed Shipment"},
