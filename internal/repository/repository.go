...
// Create writes the first version of an agreement.
//
// Version 1 is the root of its own lineage — root_agreement_id is the row's own
// id — and root_agreement_id is NOT NULL, so the id has to exist before the
// insert rather than being generated by the column default. Minting it here is
// what makes the row self-consistent in a single statement; the alternative,
// inserting and then pointing the row at itself, cannot satisfy NOT NULL on the
// way in.
//
// Revisions do NOT come through here; see CreateRevision, which needs the
// version number computed under a lock.
func (r *AgreementRepository) Create(ctx context.Context, a *models.Agreement) error {
	...
	if a.RootAgreementID == uuid.Nil {
		a.RootAgreementID = a.ID
	}
...
// Lane describes the journey a rate is being sought for.
//
// A struct rather than four positional strings because the district pair was
// added after the fact, and `FindRate(ctx, id, a, b, c, d, e)` at a call site
// is a bug waiting for someone to transpose origin and destination.
type Lane struct {
	OriginCityID      string
	DestinationCityID string

	// The kecamatan, empty for a company that prices city to city. Empty means
	// "do not narrow by district", NOT "match rates whose district is NULL":
	// see the WHERE clause below for why those differ.
...
// FindRate resolves the contracted price for a lane.
//
// A rate matches when each of its dimensions either equals the lane's value or
// is NULL, NULL being the wildcard — so an agreement can price "anything into
// Semarang" without enumerating origins. The most specific match wins.
//
// The district pair needs care that the city pair does not. When the caller
// supplies no district, district-specific rates must still be eligible:
// refusing them would mean a company that prices below city level gets no rate
// at all for an order whose warehouses have no kecamatan recorded. But when the
// caller DOES supply one, a rate for a different kecamatan must not match — it
// is a different lane at a different price. Hence the asymmetry: an empty
// district ignores the column, a present one demands equality or NULL.
func (r *AgreementRepository) FindRate(ctx context.Context, agreementID uuid.UUID, lane Lane) (*models.AgreementRate, error) {
	q := r.db.WithContext(ctx).
		Where("agreement_id = ?", agreementID).
		Where("(origin_city_id = ? OR origin_city_id IS NULL)", lane.OriginCityID).
		Where("(destination_city_id = ? OR destination_city_id IS NULL)", lane.DestinationCityID).
		Where("(truck_type_id = ? OR truck_type_id IS NULL)", lane.TruckTypeID)

	if lane.OriginDistrictID != "" {
		q = q.Where("(origin_district_id = ? OR origin_district_id IS NULL)", lane.OriginDistrictID)
	}
	if lane.DestinationDistrictID != "" {
		q = q.Where("(destination_district_id = ? OR destination_district_id IS NULL)",
			lane.DestinationDistrictID)
	}

	var rate models.AgreementRate
	err := q.
		// Prefer the most specific match: a lane-and-type rate beats a
		// wildcard, and a kecamatan lane beats the city lane containing it.
		Order(clause.OrderBy{Expression: clause.Expr{
			SQL: "(origin_city_id IS NOT NULL)::int + (destination_city_id IS NOT NULL)::int" +
				" + (truck_type_id IS NOT NULL)::int" +
				" + (origin_district_id IS NOT NULL)::int" +
				" + (destination_district_id IS NOT NULL)::int DESC",
		}}).
		First(&rate).Error
	return one(&rate, err)
}
